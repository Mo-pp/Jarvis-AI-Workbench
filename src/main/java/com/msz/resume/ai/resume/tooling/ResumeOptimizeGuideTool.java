...
public class ResumeOptimizeGuideTool {
    ...
    private static final String INSTRUCTION = """
            ...
            #### 5.6 项目经历业务化硬规则
            当用户要求结合 GitHub、源码、README、OpenViking 或 `viking://` 项目资源优化简历时，必须先把项目证据翻译成业务场景、用户流程、交付对象、业务价值和可实测指标，再重写项目经历。
            ...
            optimizedResume 的最终 bullet 允许直接写 `提升 X%`、`下降 Xms`、`减少 X 次`、`转化率提升 X%`、`采纳率提升 X%`、`质量分提升 X 分` 等待用户实测替换的占位指标。
            严禁把未实测的 X 替换成具体数字；严禁无证据编造公司、用户规模、上线范围、收入、成本、真实 QPS、真实延迟或真实百分比。
            如果 optimizedResume 中使用 X 占位指标，不要在项目描述里写“待实测指标/测量方法”；可在 suggestions 或等价字段里提示用户如何实测并替换。
