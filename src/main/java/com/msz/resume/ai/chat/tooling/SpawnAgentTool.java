 ...
 *   <li><b>subagentType</b>：子Agent类型（General/Plan/Explore），默认General</li>
...
public class SpawnAgentTool {
     ...
     * @param subagentType    子Agent类型：General(通用), Plan(规划), Explore(探索)。默认General。
 ...
 * @param maxTurns        子Agent最大迭代轮次，默认30。主Agent应根据任务复杂度主动调整。
    ...
    @Tool("将复杂子任务派发给子Agent独立执行。子Agent在隔离上下文中完成任务后返回结果摘要。" +
          ...
          "subagentType参数选择子Agent类型：General(通用读写)、Plan(规划只读)、Explore(探索只读)。" +
          "子Agent不可再派发任务或向用户提问。")
    public String spawnAgent(
            @P("任务描述，告诉子Agent需要完成什么任务") String prompt,
            @P("子Agent类型：General(通用读写), Plan(规划只读), Explore(探索只读)。默认General。") String subagentType,
